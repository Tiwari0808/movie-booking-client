...
const MovieCard = ({movie}) => {
    ...
    return (
        ...
        className='relative p-3 max-w-[268px] max-h-[356px] bg-[#12161C] rounded-[16px] flex flex-col gap-[1rem]'>
            ...
            <div><p className='font-[Heebo] text-[14px] font-semibold text-[#797B7D] align-middle'>{`${new Date(movie.release_date).getUTCFullYear()} - ${movie.genres?movie.genres.slice(0,2).map(genre=>genre.name).join(" | "):''} - ${getHours(movie.runtime)}`}</p></div>
            <div className='flex items-center justify-between mt-5'>
                ...
                <div className='flex gap-2 items-center'>
                    ...
                    <p className='text-[#797B7D] text-[16px]'>{movie.vote_average?movie.vote_average.toFixed(1):''}</p>
                </div>
            </div>
        </motion.div>
    )
